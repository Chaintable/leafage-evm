...
use crate::updater::KafkaS3Config;
...
pub async fn initialize_check<DB: EvmStorageWrite + Send + Sync + 'static>(
    ...
    if db.last_committed_block()?.is_none() {
        if let Some(rpc_address) = rpc_url.clone() {
            let mut initializer = HttpInitializer::new(db, rpc_address)?;
            initializer.init().await?;
        } else if let Some(kafka_s3_config) = kafka_s3_cfg {
            let mut initializer = KafkaInitializer::new(db, kafka_s3_config).await?;
            initializer.init().await?;
        }
    }
    Ok(())
