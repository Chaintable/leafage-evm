...
/// an enabled CacheDiskLayer, each block touching one contract with
/// SLOTS_PER_BLOCK slots and ACCOUNTS_PER_BLOCK accounts.
fn build_chain(depth: usize) -> Arc<LinkedDiffLayer> {
    let cache = Arc::new(LinkedDiffLayer::CacheDiskLayer(CacheDiskLayer::new(
        200_000, 2_000_000, 20_000, 0, true,
    )));
    let mut prev = cache;
    for n in 0..depth {
...
fn probe_keys() -> (H256, Vec<H256>) {
    ...
    let keys = (0..PROBE_KEYS as u64).map(|i| h256(NS_PROBE | (i + 1))).collect();
    (address, keys)
}
