...
use leafage_evm_chains::rsk::{is_unsupported, RskEvm, RskHardfork};
...
/// Pre-check that short-circuits a call to one of the RSK precompiles leafage
/// doesn't execute before revm validates the tx envelope (gas/nonce/balance).
/// Without this, a `gas == 0` call to a native contract — which is the on-chain
/// shape of the REMASC transaction closing every RSK block — fails with revm's
/// `CallGasCostMoreThanGasLimit` (mapped to `-39001 GasExhausted`) before
/// reaching `RskEvm::frame_init`, where the precompile guard lives. nodex-proxy
/// only retries against the RSK node on `-39008 UnsupportedPrecompile`, so the
...
type RskApiImpl<DB> = ApiImpl<DB, RskHardfork, NoneEvmCustomConfig>;

impl<DB> GasFeeHandler for RskApiImpl<DB>
where
    DB: Sync + Send + 'static,
{
    type Tx = TxEnv;
...
impl<DB> EvmExecutor for RskApiImpl<DB>
where
    DB: Sync + Send + 'static,
{
    type Tx = TxEnv;
        ...
        StateDB::Error: Sync + Send + 'static,
    {
        ...
        let mut evm = RskEvm::new(evm_env, wrap_database_ref, NoOpInspector {});
        ...
        F: FnOnce(TracingInspector) -> R,
    {
        ...
        let mut evm = RskEvm::new(evm_env, wrap_database_ref, &mut inspector);
        evm.inspect_tx_commit(tx)
            .map(|res| (res.into(), inspector_collect(inspector)))
    }
}

impl<DB> ApiCore for RskApiImpl<DB> where DB: Sync + Send + 'static {}
...
mod tests {
    ...
    /// gas=0 to a native contract must short-circuit at the executor entry
    ...
    /// RSK node.
    #[test]
    fn gas_zero_to_native_contract_returns_unsupported_precompile() {
        let remasc = address!("0x0000000000000000000000000000000001000008");
        let tx = build_tx(TxKind::Call(remasc), 0);
        let err = unsupported_precompile_error::<Infallible>(&tx)
            .expect("expected EVMError::Custom for native contract with gas=0");
        match err {
            EVMError::Custom(msg) => {
                assert!(
                ...
                assert!(msg.to_lowercase().contains(&format!("{remasc:#x}")));
            }
            other => panic!("expected Custom, got {other:?}"),
        }
    }

    /// Native contracts must be detected at the executor entry, regardless of
    /// gas value. Mirrors the `rsk/precompile.rs::UNSUPPORTED_LIST` set.
    #[test]
    fn native_contracts_short_circuit() {
        let addrs: [Address; 3] = [
            address!("0x0000000000000000000000000000000001000006"), // Bridge
            address!("0x0000000000000000000000000000000001000008"), // REMASC
            address!("0x0000000000000000000000000000000001000010"), // BlockHeader
        ];
        for addr in addrs {
