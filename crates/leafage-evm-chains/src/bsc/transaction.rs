use alloy::consensus::{EthereumTxEnvelope, EthereumTypedTransaction, TxEip4844};
...
impl FromTxWithEncoded<TransactionSigned> for BscTxEnv {
    fn from_encoded_tx(tx: &TransactionSigned, sender: Address, _encoded: Bytes) -> Self {
        let base = match tx.clone().into_typed_transaction() {
            EthereumTypedTransaction::Legacy(tx) => TxEnv::from_recovered_tx(&tx, sender),
            EthereumTypedTransaction::Eip2930(tx) => TxEnv::from_recovered_tx(&tx, sender),
            EthereumTypedTransaction::Eip1559(tx) => TxEnv::from_recovered_tx(&tx, sender),
            EthereumTypedTransaction::Eip4844(tx) => TxEnv::from_recovered_tx(&tx, sender),
            EthereumTypedTransaction::Eip7702(tx) => TxEnv::from_recovered_tx(&tx, sender),
        };
