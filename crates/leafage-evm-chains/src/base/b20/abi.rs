...
//!
//! Transcribed from Base reth v1.4.2 (`common/abi/{v1,v2}.rs`, `b20_asset/abi/{v1,v2}.rs`,
//! `b20_stablecoin/abi.rs`). Selector sets must stay byte-identical to Base's: an interface
//! that is missing a selector reverts a call Base would execute, and one that adds a selector
//! executes a call Base would revert — either way the gas diverges.
//!
//! As in Base, the unsuffixed interfaces are the canonical (latest, Cobalt) surface that calls
//! are decoded into, and the `V1` interfaces are the frozen Beryl surface. A Beryl-era call is
//! first gated on the frozen surface — which is what rejects the Cobalt selectors and the
//! `SEIZE` pausable feature before Cobalt — and then decoded canonically.
...
/// ERC-165 `supportsInterface` interface ID.
pub const ERC165_INTERFACE_ID: alloy::primitives::FixedBytes<4> =
    alloy::primitives::FixedBytes::new([0x01, 0xff, 0xc9, 0xa7]);

/// ERC-8056 interface IDs the Cobalt asset advertises: `IScaledUIAmount`,
/// `IScaledUIAmountNewUIMultiplier`, `IScaledUIAmountBalances`, `IScaledUIAmountConversion`.
pub const ERC8056_INTERFACE_IDS: [alloy::primitives::FixedBytes<4>; 4] = [
    alloy::primitives::FixedBytes::new([0xa6, 0x0b, 0xf1, 0x3d]),
    alloy::primitives::FixedBytes::new([0x4b, 0xd2, 0x76, 0x48]),
    alloy::primitives::FixedBytes::new([0xd8, 0x90, 0xfd, 0x71]),
    alloy::primitives::FixedBytes::new([0x57, 0x85, 0x4f, 0xc3]),
];
