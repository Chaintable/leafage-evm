...
/// Item values keep the JSON shape of the corresponding single method:
/// `getStorageAt` returns a 32-byte hex word, `getAddressCode` returns
/// variable-length hex bytes. Untagged so both serialize as plain hex
/// strings on the wire.
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Eq)]
#[serde(untagged)]
pub enum BlockxStateReadValue {
    Storage(H256),
    Code(Bytes),
}
