...
- `[2026-07-21][decided] 剩余 Stylus 验证缺口:影响不小但当前触发概率极低,暂缓` — 深入推演后定级,不再逐项追。
  **风险评估（影响 × 概率）:** 影响都落在共识 gas / 状态正确性上，不算小；但**概率**极低——hood 主网无 Stylus 合约；Arb One 只有 13 个程序、行为简单，实测 op 序列里连一次 SSTORE 都没有，更没有嵌套调用。
  **暂缓项:** ① `create1/create2`、`emit_log`、`account_balance/code/codehash` 零执行；② **storage 写入 + SSTORE refund 全链路零执行**（`journal.sstore` 与 refund 传到 `reward_beneficiary` 的链路是 revm 既有机制、推演成立，但没跑过，`sstore_refund` 的符号处理尤其只是纸面）；③ **嵌套 Stylus（A→B 都是 Stylus）**；④ 顶层 trace 帧的 gas 归因口径（nitro 含子树 38556 / leafage 仅自身 17225，外层 gas_used 一致）。
  **已推演排除、不必再查的:** driver 在递归中沿 override 自然传播（Traced/Plain 各自硬编码）；trace 平衡有 revm 源码依据（`inspect_frame_run` 自己 fire `frame_end`+`set_finished`，Stylus 帧的 `set_finished` 由 `finish_frame` 负责）；执行上下文 per-tx 新建不跨调用泄漏（`simulateTransactions` 循环里每 tx 走 `execution_env_for_tx`，只共享 `memory_db`）；`set_trie_slots` 的 checkpoint revert 不会错误撤销 warm 状态（走到该分支后程序立即 OOG 中止，无后续访问）。
  **③ 的真正风险点不在 frame 管理**（memory context 分属不同 `SharedMemory`、`parent_index` 守卫成立、reentrant 计数正确、static/delegate 上下文正确传播），而在 **libstylus 的 FFI 重入**：A 的 `stylus_call` 未返回时栈上又发起 B 的 `stylus_call`。nitro 自己支持嵌套且同样是同步 C ABI 调用，故推断安全，但这在 leafage 边界之外，读代码无法证明；另有 wasmer 每层占原生栈的深度问题（`max_depth` 已传，未试边界）。
  **重新评估的触发条件（任一成立就得回来做）:** ① hood 或目标链出现真实 Stylus 合约，尤其是会**写 storage**、**跨合约调用**或 **CREATE** 的；② 要把 leafage 接到 Stylus 生态活跃的链（Arb Sepolia 等）；③ 有人报 Stylus 相关的 gas 或状态不一致。届时最省力的切入点是 ②——`simulateTransactions` 本就是 commit 语义，找一个写 storage 的 Stylus 调用即可直接对 writer 比 state diff。
