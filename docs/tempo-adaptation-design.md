...
| 交易类型 | Legacy/EIP-1559/4844/7702 | 新增 TempoTransaction (type 0x76) — AA tx | **未适配（方案已确认）** — eth_call 模式下 fee/签名自动跳过，只需批量执行+2D nonce，~200 行改动，见 todo.md |
| Fee 机制 | ETH gas fee | TIP-20 fee + AMM 兑换 | **跳过** — `disable_base_fee=true`，与 writer 端 `pre_traceMany` 行为一致 |
...
**分支:** `feature/tempo-chain-adaptation` (10 commits, ~12,500 行新增代码)
...
| 全量编译 | done | `cargo check --workspace` 通过 (Rust 1.93.0) |
...
| 差异点 | Tempo writer (eth_estimateGas) | leafage (estimateGas) |
|--------|-------------------------------|----------------------|
| gas 上界 | TIP-20 余额 * SCALING_FACTOR / gas_price | rpc_gas_cap (固定值，默认 100M) |
| 执行路径 | 完整 TempoEvmHandler (含 fee 预扣/退还) | 标准 EVM (disable_base_fee) |
| 结果 | 包含 handler 层 fee overhead | 不含 fee overhead，偏低 |

### 7.2 TempoTransaction (type 0x76) 批量执行未实现

leafage 当前接收标准 `CallRequest`，不支持 AA tx 的 `Vec<Call>` 批量原子执行。DeBankCore 需将 AA tx 内部参数提取为独立 CallRequest。

**已确认实现方案（~200 行改动）：** 通过分析 Tempo handler 源码确认，eth_call 模式下 fee handler 和签名验证**自动跳过**。只需：
- `TempoTxEnv` 包装 TxEnv + aa_calls/nonce_key 字段
- `TempoHandler::execution()` override — 批量原子执行（checkpoint → 逐 call → revert on failure）
- `create_txn_env` 2D nonce 从 NonceManager storage 读取
- `CallRequest` 加 `tempo_calls` / `nonce_key` 可选字段

详见 `docs/todo.md` "TempoTransaction 批量执行实现方案"章节。
...
- [ ] **estimateGas fee overhead** — 如 DeBankCore 需要精确 gas 值，需实现 TempoEvmHandler

### P2 — 按需

- [ ] **TempoTransaction (0x76)** 完整支持 — 自定义 TxEnv + Handler + 签名验证 (BSC 适配量 3-5x)
