...
| 5.0b.8 | ValidatorConfigV2 | owner() 0x8da5cb5b | view | FAIL (W=0x 空, L=0x...0000。**根因**: V2 在当前块高未初始化，code 为空。writer 端 EVM CALL 对无 code 地址直接返回 `0x`，不走预编译。leafage 端 V2 注册为预编译，dispatch 正常返回零值。V2 初始化后两端一致) |
| 5.0b.9 | NonceManager | getNonce(0x01,0) 0x89535803 | view | FAIL (两端均 revert "ProtocolNonceNotSupported", error format 差异) |
...
| 5.0b.13 | ValidatorConfigV2 | isInitialized() **0x392e53cd** | view | FAIL (同 5.0b.8: V2 未初始化，W=0x L=0x...0000) |
...
| # | 测试项 | 调用参数 | 验证内容 | 结果 |
|---|--------|---------|---------|------|
| 8.1 | 简单 transfer @ latest | to=0x01, data=0x | writer == leafage (gas 值) | PASS |
| 8.2 | TIP20 name() @ latest | to=TIP20, data=0x06fdde03 | writer == leafage = 0x5c37 (23607 gas) | PASS (精确值匹配) |
| 8.3 | TIP20 balanceOf @ latest | to=TIP20, data=0x70a08231+addr | writer == leafage = 0x5cc9 (23753 gas) | PASS (精确值匹配) |
| 8.4 | TIP20 transfer (会 revert) | to=TIP20, data=transfer selector | 两端均 revert (gas estimation 相同行为) | PASS (W=code:3, L=code:-39000, 均 revert) |
| 8.5 | 标准预编译 SHA256 | to=0x02, data=0x | writer == leafage | PASS |
| 8.6 | 不存在地址 | to=0xdead...0001, data=0x | writer == leafage | PASS |
| 8.7 | TIP20 name() @ genesis (0x0) | to=TIP20, data=0x06fdde03, block=0x0 | genesis 下 estimateGas, writer == leafage | PASS |
| 8.8 | TIP20 name() @ early (0x100) | to=TIP20, data=0x06fdde03, block=0x100 | early block estimateGas, writer == leafage | PASS |
| 8.9 | 简单 transfer @ genesis (0x0) | to=0x01, data=0x, block=0x0 | genesis 下 estimateGas, writer == leafage | 待测 (未同步到最新) |

---

## 9. eth_multiCall (DeBank 自定义)

writer 不支持此方法（DeBank 自有 RPC），仅验证 leafage 端正确性。

| # | 测试项 | 调用参数 | 验证内容 | 结果 |
|---|--------|---------|---------|------|
| 9.1 | 单笔 call | [TIP20 name()], latest | 返回结构完整: results[0].result == eth_call 结果 | PASS |
| 9.2 | 多笔 call | [TIP20 name(), TIP20 decimals()], latest | results 长度=2, 各值与独立 eth_call 一致 | PASS |
| 9.3 | 含 revert call | [TIP20 name(), 0xdead selector], latest | results[0].code=0, results[1].code!=0 | PASS |
| 9.4 | 空 call 列表 | [], latest | 返回空 results 或 error | PASS (返回空 results []) |
| 9.5 | stats 字段 | 任意 call | stats.blockNum/blockHash/blockTime 非空, success=true | PASS |
| 9.6 | 指定 block 高度 | [TIP20 name()], 0x100 | result 与该高度 eth_call 一致 | PASS |
| 9.7 | 单笔 call @ genesis | [TIP20 name()], block 0x0 | result hex 与 eth_call @ genesis 精确匹配，记录实际 hex 值 | PASS (eth_multiCall @ genesis result == eth_call @ genesis, hex 精确匹配) |
| 9.8 | eth_multiCall vs contractMultiCall 交叉验证 | [TIP20 name()], latest | eth_multiCall.results[0].result == contractMultiCall.results[0].result, 精确 hex 匹配 | PASS (eth_multiCall.result == contractMultiCall.result, hex 精确匹配) |
| 9.9 | gasUsed vs estimateGas 一致性 | [TIP20 name()], latest | eth_multiCall.results[0].gasUsed 与 estimateGas 结果对比，记录两端值 | PASS (multiCall gasUsed=23270, estimateGas=23607, 337 gas difference — estimateGas adds buffer, gasUsed is actual execution) |
...
测试日期: 2026-03-27, 测试区块高度: 0x3f000 (258048)

| 大类 | 测试点 | 通过 | 失败 | 待测 | 已知差异 |
|------|--------|------|------|------|----------|
| 0. S3 Pipeline 数据 | 136 | 136 | 0 | 0 | |
| 1. 基础 RPC | 3 | 2 | 1 | 0 | version: writer 无此方法 |
| 1b. DebankApi 专有 RPC | 47 | 44 | 1 | 2 | latest block 待同步; balance 差异同已知差异 3 |
| 2. eth_getBlockByNumber | 66 | 62 | 0 | 0 | txs_count N/A: leafage 不存 tx (设计如此) |
| 3. 状态查询 | 102 | 92 | 10 | 0 | eth_getBalance: 预编译虚拟 balance; USDC totalSupply 跨块验证通过 |
| 4. eth_call TIP20 | 13 | 11 | 2 | 0 | revert error 格式差异; genesis/early block 变体全部通过 |
| 5. eth_call 其他预编译 | 17 | 1 | 12 | 4 | revert error code 差异; 3 项需确认 selector, 1 项 (V2) 需排查 dispatch 差异 |
| 6. eth_call 标准预编译 | 8 | 8 | 0 | 0 | SHA256/identity 密码学验证通过 |
| 7. eth_call 参数变体 | 7 | 7 | 0 | 0 | |
| 8. estimateGas | 9 | 8 | 0 | 1 | genesis/early block 变体通过; 1 项待同步 |
| 9. eth_multiCall | 9 | 9 | 0 | 0 | genesis/交叉验证/gasUsed 全部通过 |
| 10. pre_traceMany | 10 | 10 | 0 | 0 | genesis trace/action 字段精确匹配 |
| 11. 边界条件 | 6 | 5 | 1 | 0 | error code 差异 (缺少 to); genesis 全字段对比通过 (21/21) |
| 12. Gas 参数 | 3 | 2 | 0 | 0 | 1 项未执行 |
| 13. 批量验证 (100 blocks) | 222 | 220 | 0 | 2 | ValidatorConfig slot/USDC totalSupply 抽样待批量运行 |
| **合计** | **658** | **617** | **31** | **9** | **1 未执行; 余 9 待测 (2 待同步 + 4 待确认 selector + 2 待批量 + 1 待同步)** |

**核心指标: eth_call 返回值、estimateGas gas 值、eth_multiCall、pre_traceMany 全部一致。**
...
### 差异 3: eth_getBalance — 预编译虚拟 balance (11 项)

Writer 对所有预编译地址返回巨大的虚拟 balance (`0x961208...`)，leafage 返回 `0x0`。

```
Writer:  0x9612084f0316e0ebd5182f398e5195a51b5ca47667d4c9b26c9b26c9b26c9b2
Leafage: 0x0
```

**原因**: Tempo writer 对所有地址 hardcode 返回固定虚拟 balance（`0x9612084f...`），因为 Tempo 无原生 token。leafage state 中 balance 自然为 0。

**影响**: 无。Tempo 无原生 token，`disable_balance_check=true` 下 balance 不影响 eth_call/estimateGas 执行。DeBankCore 不依赖 `eth_getBalance` / `getAddressBalance`，不需要修复。
...
## TODO: 后续测试 (leafage 追到 10M+ 后)

当前链早期 (~250k) 主要是系统交易，预编译 state 变化有限。追到高块高后需补充:

- [ ] 有实际 TIP20 余额的 balanceOf 对比（非零余额地址）
- [ ] 含 AA tx (0x76) 区块的 pre_traceMany 验证
