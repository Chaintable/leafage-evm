...
### 9e. AA 0x76 batch — pre_traceMany + simulateTransactions

验证 0x76 AA batch calls 在 pre_traceMany 和 simulateTransactions 中正确执行。
使用真实 0x76 tx (0x68793652..., block 0xbad463, 3 calls: approve + 2x swap)。

| # | 测试项 | 方法 | 验证内容 | 结果 |
|---|--------|------|---------|------|
| 9.21 | pre_traceMany AA batch (writer vs leafage) | pre_traceMany with calls+nonceKey @0xbad462 | gasUsed + traces count 完全匹配 | PASS (W=2536698, L=2536698, traces=24/24) |
| 9.22 | simulateTransactions AA batch | simulateTransactions with calls+nonceKey @0xbad462 | code=0, gasUsed, traces 正确 | PASS (gasUsed=2536698, traces=24) |
| 9.23 | simulateTransactions vs pre_traceMany AA | 对比 9.21 和 9.22 | gasUsed 完全一致 | PASS |
| 9.24 | simulateTransactions vs debug_traceTransaction | 对比 simulate gasUsed vs on-chain gasUsed | 已知差异：simulate 比 on-chain 多 ~15000 gas (fee collection warm-up) | KNOWN_DIFF (+15000, 3 calls) |

**已知差异说明**：simulateTransactions/pre_traceMany 的 gasUsed (2536698) 比链上实际执行 gasUsed (2521698) 多 15000。
原因：writer 在 block execution 时先执行 `collectFeePreTx`（TIP20 fee transfer），暖化了 transfer_policy_id、TIP403 policy 等 storage slots。
leafage RPC 不执行 fee collection，这些 slots 在首次 batch call 时为 cold（2100 gas vs warm 100 gas）。
此差异是架构设计导致的（读节点不做 fee collection），不影响 estimateGas 或 contractMultiCall 的准确性。

### 9f. simulateTransactions vs pre_traceMany 全量交叉验证

三方对比：Writer pre_traceMany / Leafage pre_traceMany / Leafage simulateTransactions。
Block: 0xbad462 (12244066)。**Leafage sim vs Leafage ptm: 25/25 一致 (100%)**。

**Group A: 标准 TIP20/预编译调用**

| # | 测试项 | W ptm | L ptm | L sim | 一致 |
|---|--------|-------|-------|-------|------|
| 9.25 | TIP20 name() | 273270 | 273270 | 273270 | YES |
| 9.26 | TIP20 decimals() | 271170 | 271170 | 271170 | YES |
| 9.27 | TIP20 balanceOf(user) | 273644 | 273644 | 273644 | YES |
| 9.28 | TIP20 totalSupply() | 273270 | 273270 | 273270 | YES |
| 9.29 | USDC name() | 273270 | 273270 | 273270 | YES |
| 9.30 | FeeManager M() (revert) | 0 | 271070 | 271070 | PARTIAL (W=0 on revert) |
| 9.31 | ValidatorConfig validatorCount() | 271064 | 271064 | 271064 | YES |
| 9.32 | SHA256 precompile | 271152 | 271152 | 271152 | YES |
| 9.33 | invalid selector (revert) | 0 | 271170 | 271170 | PARTIAL (W=0 on revert) |

**Group B: 真实链上 0x76 AA batch**

| # | 测试项 | W ptm | L ptm | L sim | 一致 |
|---|--------|-------|-------|-------|------|
| 9.34 | 3-call batch (approve+2x swap) | 2536698 | 2536698 | 2536698 | YES |
| 9.35 | 2-call batch (approve+swap) | 1481134 | 1481134 | 1481134 | YES |
| 9.36 | 1-call USDC (expiring nonce) | 0 | 67968 | 67968 | PARTIAL (W=0 validation error) |
| 9.37 | 2-call batch (approve+swap) | 1481134 | 1481134 | 1481134 | YES |

**Group C: 构造 AA batch**

| # | 测试项 | W ptm | L ptm | L sim | 一致 |
|---|--------|-------|-------|-------|------|
| 9.38 | 1 call: name() | 23270 | 23270 | 23270 | YES |
| 9.39 | 2 calls: name()+decimals() | 26040 | 26040 | 26040 | YES |
| 9.40 | 3 calls: name()+decimals()+totalSupply() | 30910 | 30910 | 30910 | YES |
| 9.41 | 5 calls: mixed view | 39024 | 39024 | 39024 | YES |
| 9.42 | 2 calls: transfer (revert) | 310660 | 310660 | 310660 | YES |
| 9.43 | 2 calls: name()+invalid (revert) | 0 | 26040 | 26040 | PARTIAL (W=0 on revert) |

**Group D: 边界**

| # | 测试项 | W ptm | L ptm | L sim | 一致 |
|---|--------|-------|-------|-------|------|
| 9.44 | system tx (gas=0x1c9c380) | 271028 | 271028 | 271028 | YES |
| 9.45 | high gas single AA call | 23270 | 23270 | 23270 | YES |
| 9.46 | batch: standard name() | 273270 | 273270 | 273270 | YES |
| 9.47 | batch: standard decimals() (warm) | 21170 | 21170 | 21170 | YES |
| 9.48 | batch: AA totalSupply() | 23270 | 23270 | 23270 | YES |

**已知差异**：Writer pre_traceMany 对 revert/validation error 返回 gasUsed=0，Leafage 返回实际消耗 gas。仅影响报告格式，不影响执行正确性。
