...
leafage-evm 是一个基于 [alloy](https://github.com/alloy-rs/alloy) 和 [revm](https://github.com/bluealloy/revm) 实现的轻量级 EVM 执行器。专注于**状态查询**（`eth_call`、`estimateGas` 等），**不存储交易数据**。通过 Kafka + S3 获取状态更新，而非 P2P 同步。
