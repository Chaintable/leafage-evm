...
leafage-evm 是一个基于 Rust 和 [revm](https://github.com/bluealloy/revm) 实现的轻量级 EVM 执行器。通过 Kafka + S3 获取状态更新，而非 P2P 同步。
...
- **HTTP（备用方式）**：轮询修改版 Geth 的 `trace_blockStateDiff` RPC 接口
